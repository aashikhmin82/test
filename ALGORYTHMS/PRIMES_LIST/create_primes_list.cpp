...
//    auto primes_list1 = create_primes_list1(max_num, debug);
    ...
    using duration_t = decltype((end_time - start_time).count());
    vector<duration_t> func_durations(func_amount);
    func_durations[0] = (end_time - start_time).count();
...
//    auto primes_list2 = create_primes_list2(max_num, debug);
    ...
    func_durations[1] = (end_time - start_time).count();
...
//    auto primes_list3 = create_primes_list3(max_num, debug);
    ...
    func_durations[2] = (end_time - start_time).count();
...
int main(int argc, char** argv)
{
    ...
    cout << "Max Number\t" 
        << "Function 1 (Check all numbers)\t"
        << "Function 2 (Using std::set)\t"
        << "Function 3 (std::vector<bool>)\t"
        << "(Min Value)" << endl;
    for (const auto max_num : max_num_list)
    {
        ...
        cout << max_num << "\t"
            ...
            << func_durations[1] << "\t"
            << func_durations[2] << "\t"
            << "(Func " << (minmax.first - func_durations.begin() + 1) << ")" << endl;
    }
