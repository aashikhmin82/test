...
size_t sort_by_pivot(const size_t min_i, const size_t max_i, vector<size_t>& test_vec, print_debug& debug) {
    size_t pivot_i = get_random_it_num(min_i, max_i);
    debug << "[D] Pivot : " << pivot_i << "  (" << test_vec[pivot_i] << ")\n";
    debug << "[D sort_by_pivot] min : " << min_i << "  max : " << max_i << "\n";
...
void sort_thread(const size_t min_i, const size_t max_i, vector<size_t>& test_vec, print_debug& debug) {
    ...
    if (min_i <= max_i) {
        if (min_i != max_i) {
            size_t wall = sort_by_pivot(min_i, max_i, test_vec, debug);
...
void divide_vec_to_parts(auto& test_vec, auto& walls_vector, size_t threads_amount, auto& debug) {
    ...
    while (thread_i < threads_amount and wall < test_vec.size() - 1 ) {
        ...
        if (thread_i == 2) {
            wall = sort_by_pivot(0, test_vec.size() - 1, test_vec, debug);
            ...
            if (wall_i == walls_vector_size_prev) {
                ...
                walls_vector_tmp.clear();
                walls_vector_tmp.push_back(0);
                min = 0;
            } else {
                ++wall_i;
            }
            ...
            wall = sort_by_pivot(min, max, test_vec, debug);
