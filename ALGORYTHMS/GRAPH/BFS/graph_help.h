...
//    class_graph_debug  operator<<(string text)
    template <typename Type> class_graph_debug  operator<<(Type text)
    {
        if (debug_flag)
                    cout << text;
    ...
    const void print_debug_graph(map<string, vector<string> >& graph_map, map<string, class_graph_element*>& graph_objects_list)
    {
        if (debug_flag)
        {
    ...
    const void print_debug_result(map<string, vector<string> >& graph_map, map<string, class_graph_element*>& graph_objects_list)
    {
        if (debug_flag)
        {
            ...
            for (const auto& i : graph_map)
            {
                cout << "[D]<print_debug_result> " << graph_objects_list[i.first]->value() << " (" << graph_objects_list[i.first]->colour() << " " << graph_objects_list[i.first]->steps_count()<< ") : ";
                for (const auto& j : i.second)
                {
